...
##############################################################################
# 
# Assemble your Phase 2 IPsecRules from the above libraries:
#
...
* When creating an IKEv2 rule, you might not get an error in PoSh, the rule might 
become visible in the WF snap-in, but the Security log can show an error that the 
rule was not applied (Category: MPSSVC Rule-Level Policy Change, ID: 4958).
Or, there might be no error in the Security log and the rule will simply
...
ConSecRule in the WF snap-in -- again, a rule which does nothing).
