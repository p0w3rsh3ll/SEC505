...
$MmAes128Sha256Dh14 = New-NetIPsecMainModeCryptoProposal -Encryption AES128 -Hash SHA256 -KeyExchang DH14
...
$MmCryptoSetAllowDH = New-NetIPsecMainModeCryptoSet -DisplayName 'MMCryptoSetAllowDH' -ForceDiffieHellman $False -Proposal $MmAes128Sha256Dh14
#The one above will not appear in the WF, only the next one!
$MmCryptoSetForceDH = New-NetIPsecMainModeCryptoSet -DisplayName 'MMCryptoSetForceDH' -ForceDiffieHellman $True  -Proposal @($MmAes256Sha384Dh24,$MmAes128Sha256Dh14) 
...
before creating new ones, then use the -Name parameter to help find duplicates. 
If you try to create a duplicate IPsec object with the same name as an existing
object, you will get a "Catastrophic failure" error.  

* Just because intellisense shows an argument for a parameter does not mean
that that argument is always legal or will result in a functioning policy.
