...
#
#   TODO: Test for MS-Loopback, use that for $NIC.
###################################################################
...
# Confirm there is at least one adapter up:
...
{ 
    ...
    {
        $Top.Request = "Stop"
        Throw "ERROR: Could not enable a network adapter, check VM settings."
    } 
}
...
# Get first network adapter, if multiple, and $NIC will be down:
$NIC = $null
$NIC = Get-NetAdapter -Physical | Select-Object -First 1 
