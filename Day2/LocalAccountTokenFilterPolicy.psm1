...
.DESCRIPTION
 ...
 local user account on that computer (not domain accounts in Active
 Directory) will have the full powers and privileges of any other
 member of the Administrators group on that computer.  By default,
 Windows will strip the Security Access Token (SAT) associated with
 the logon session of a user account when 1) that account is a local
 account, 2) it is not the built-in local Administrator account, and
 3) that local account is in the Administrators group.  However, this
 makes it very difficult to remotely manage computers using local 
 administrative accounts, which is safer, ironically, than doing 
 remote administration with domain accounts, when the passwords of 
 the local administrative accounts are randomized.

.NOTES
 Legal: Script provided "AS IS" without warranties of any kind.
 Redistribution: Public domain, no rights reserved.
 Author: Enclave Consulting LLC, Jason @ www.sans.org/SEC505 
#>
